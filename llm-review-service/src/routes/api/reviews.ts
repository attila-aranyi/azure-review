...
import { createFeedbackRepo } from "../../db/repos/feedbackRepo";
...
export const registerReviewRoutes: FastifyPluginAsync<{
  ...
  // POST /api/reviews/:id/findings/:findingId/feedback
  app.post<{ Params: { id: string; findingId: string } }>("/reviews/:id/findings/:findingId/feedback", async (request, reply) => {
    const tenantId = request.tenantId;
    if (!tenantId) return reply.code(401).send({ error: "Unauthorized" });
    ...
    const findings = await reviewRepo.findFindingsByReviewId(review.id);
    const finding = findings.find((f) => f.id === request.params.findingId);
    if (!finding) {
    ...
    const feedback = await feedbackRepo.create({
      ...
      tenantId,
      adoUserId: (request as { adoUserId?: string }).adoUserId,
